...
function downloadAgent() {
	# Agent.ova is a Windows 7 (x86) virtual machine file and is used as a Cuckoo agent
	url="https://drive.google.com/u/0/uc?id=1uGxNwvSuSIhokeuX9N61D8VtyFDoK0-2&export=download"
	# Download Agent.ova from google drive (Manually download if not working)
	wget $url -O ~/Downloads/Agent.ova
}

function installDependencies() {
	# Install system updates and update system dependencies
	...
	# Install basic system dependencies by Cuckoo required (if an iptables-persistent configuration GUI window pops up, keep the default selection and proceed)
	...
	# Uninstall Werkzeug and reinstall Werkzeug 0.16.1 (Werkzeug has a version update which leads to incompatibility with Cuckoo 2.0.7)
	sudo pip uninstall --yes werkzeug && sudo pip install werkzeug==0.16.1
	# Install the pySSDeep, yara, and volatility plugins
...
function configureVirtualbox() {
	# Use VboxManage to create a hostonly ethernet adapter vboxnet0
	vboxmanage hostonlyif create && vboxmanage hostonlyif ipconfig vboxnet0 --ip 192.168.56.1 --netmask 255.255.255.0
	# Modify the default storage directory and permission for virtual machine files
	...
	# Import Agent.ova, boot up to take a snapshot, add it to Cuckoo and revert the snapshot
...
function configureNetwork() {
	...
	# Configure iptables to enable IP forwarding
...
function configCuckoo() {
	# Start the MongoDB service and Cuckoo, then pull community signatures
